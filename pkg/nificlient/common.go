...
var ErrNoNodeClientsAvailable = errors.New("Cannot create a node client to perform actions. There are no nodes available.")

func errorGetOperation(rsp *http.Response, body *string, err error, log *zap.Logger) error {
	if rsp != nil && rsp.StatusCode == 404 {
		log.Debug("404 response from nifi node",
	...
	if rsp != nil && rsp.StatusCode != 200 {
		log.Info("Non 200 response from nifi node",
			zap.String("url", rsp.Request.URL.String()),
			zap.String("statusCode", rsp.Status),
			zap.Stringp("body", body))
	...
	if err != nil || rsp == nil {
		log.Debug("Error during talking to nifi node",
			zap.Error(err))
		return err
	}
	return nil
